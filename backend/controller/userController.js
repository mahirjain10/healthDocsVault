...
const { setCookie } = require("../utils/cookies.js");
  ...
  try {
    ...
    if (!compareHashPassword) {
      return sendResponse(res, 401, {
        message: "Unauthorized,Password invalid",
      });
    }
...
exports.updateUserDetails = async (req, res, next) => {
  ...
  try {
    ...
    const findUserDetails = await userDetailsModel.findOneAndUpdate(
      { user: user_id },
      { name, age, gender }
    );
...
exports.getUserDetails =async (req, res, next) => {
  ...
  try {
    ...
    return sendResponse(res,200,{message:"User details fetched successfully"});
  } catch (error) {
    next(error);
  }
};
